<script lang="ts">
  ...
  function openSheet(cat: "exercise" | "music" | "content") {
    sheetCat = cat;
    sheetOpen = true;
    logError = "";
  }
...
<div
  class="max-w-[420px] mx-auto h-screen flex flex-col bg-bg border-x border-border relative"
>
  <div
    class="flex items-center justify-between px-[18px] pt-[18px] pb-3 border-b border-border flex-shrink-0"
  >
    <div>
  ...
  <div class="flex-1 overflow-hidden relative">
    {#if $entriesError}
      <div
        class="absolute top-0 inset-x-0 z-10 bg-music/10 border-b border-music/30 px-4 py-2 text-music text-xs text-center"
      >
    ...
    {#if activeTab === "hoy"}
      <div class="h-full overflow-y-auto px-4 pt-4 pb-24 animate-screen-in">
        <div class="border border-border rounded bg-panel p-4 mb-3">
                ...
                >
                  ...
                  <span class="flex-1">
                    <span
                      ...
                      >{label}</span
                    >
                    <span class="block text-[11px] font-mono text-muted mt-0.5">
                      {todayEntry
  ...
  <div
    class="flex border-t border-border bg-panel/80 backdrop-blur-sm flex-shrink-0"
  >
    {#each [{ id: "hoy", label: "Hoy", icon: "clock" }, { id: "historial", label: "Historial", icon: "calendar" }, { id: "stats", label: "Stats", icon: "chart" }] as tab}
      <button
        id="tab-{tab.id}"
        ...
        class="
          ...
          {activeTab === tab.id ? 'text-text' : 'text-muted hover:text-muted2'}
        "
      >
        {#if tab.icon === "clock"}
          <svg
  ...
  {#if sheetOpen}
    <div
      ...
      aria-modal="true"
      tabindex="-1"
      class="absolute inset-0 bg-black/45 flex items-end z-10"
      onclick={(e) => {
    ...
    >
      <div
        class="w-full bg-[rgba(24,27,32,0.6)] backdrop-blur-xl border-t border-white/8 rounded-t-[18px] px-[18px] pt-5 pb-7"
      >
